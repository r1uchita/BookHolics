    ...
    if(!in.fail())  {
        ...
        for(int j=0;!in.eof();j++){
            if(status==QString::fromStdString(book.status)) {
                ...
                ui->Details_table->setItem(i,2,new QTableWidgetItem(b_author));
                ui->Details_table->setItem(i,3,new QTableWidgetItem(b_publication));
                ui->Details_table->setItem(i,4,new QTableWidgetItem(b_edition));
                ui->Details_table->setItem(i,5,new QTableWidgetItem(b_isbn_no));
                ui->Details_table->setItem(i,6,new QTableWidgetItem(Release_Year));
                i++;
            }
            in>>book;
        }
        ...
        QMessageBox *msg;
        msg->information(this,"fail","Books Not Available");
        in.close();
    }
}
