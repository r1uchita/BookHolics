...
void book_returned::on_pushButton_clicked() {
    ...
    if((ui->from_date->selectedDate().isNull() || ui->to_date->selectedDate().isNull()) && ui->comboBox_member->currentText()=="Select") {
        ...
        if(ui->comboBox_member->currentText()=="Student")   {
            int i=0,flag=0;
            book_return book;
            ui->return_deatils->setColumnCount(11);
            std::ifstream in;
            ...
            if(!in.fail())   {
                ...
                for(int j=0;!in.eof();j++){
                    ...
                    if(Date>=to_date && Date<=from_date && book.borrow_id==ui->lineEdit_borrow_id->text().toInt()) {
                          ...
                          QString return_date=QString::fromStdString(book.return_date);
                          QString fine=QString::number(book.fine);
                          ...
                          ui->return_deatils->setItem(i,9,new QTableWidgetItem(return_date));
                          ui->return_deatils->setItem(i,10,new QTableWidgetItem(fine));
                          i++;
                          flag=1;
                      }
                      in>>book;
                ...
                if(flag==0)
                    QMessageBox::warning(this,"Warning","Given Borrow id Student Return any book");
                in.close();
            }
            else {
        ...
        int i=0;
        book_return book;
        ui->return_deatils->setColumnCount(8);
        std::ifstream in;
        ...
        if(!in.fail())  {
            ...
            for(int j=0;!in.eof();j++){
                ...
                if(Date>=to_date && Date<=from_date && book.borrow_id==ui->lineEdit_borrow_id->text().toInt()) {
                      ...
                      ui->return_deatils->setItem(i,5,new QTableWidgetItem(s_name));
                      ui->return_deatils->setItem(i,6,new QTableWidgetItem(s_dept));
                      ui->return_deatils->setItem(i,7,new QTableWidgetItem(return_date));
                      i++;
                      flag=1;
                  }
                  in>>book;
