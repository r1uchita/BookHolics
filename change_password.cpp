...
void change_password::change_pass_stud(){
    ...
    if(flag==1){
        ...
        QMessageBox::information(this,"Change Password","Please Enter Correct Password...!");
    }
}
...
{
    if(check_digit(ui->new_pass->text()) && check_lower(ui->new_pass->text()) && check_upper(ui->new_pass->text())){
        ...
        QMessageBox::information(this,"Error","Password is weak\nPassword should be contain combination of\n Number,Special symbol,capital letters,Lower letters");
    }
}
bool change_password::check_lower(QString password ){
   int count;
...
bool change_password::check_digit(QString password){
    int count;
    const char *pass=password.toStdString().c_str();
    bool flag=false;
    for(count=0;count<password.length();count++){
...
bool change_password::check_upper(QString password){
    int count;
    const char *pass=password.toStdString().c_str();
    bool flag=false;
    for(count=0;count<password.length();count++){
