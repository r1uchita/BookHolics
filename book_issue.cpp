...
void book_issue::on_lineEdit_book_id_editingFinished() {
    ...
    if(!ui->lineEdit_book_id->hasAcceptableInput()) {
        ...
        if(!in.fail())  {
            in>>book;
            while(!in.eof()){
                if(id==book.book_id&&book.status=="Issued") {
                    flag=2;
                    break;
                }
                else if(id==book.book_id && book.status=="Available") {
            ...
            in.close();
            if(flag==0){
                QMessageBox::information(this,"Fail","This Book is Not Available");
            }
            else if(flag==2){
                QMessageBox::information(this,"warnning","Book is Already Issued");
            }
       }
       else {
